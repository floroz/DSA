...
export default function getElementsByClassName(
  ...
  const classes = Array.from(new Set(classNames.split(" ")))
    // clear whitespace
    .map((cls) => cls.trim())
    // remove empty
    .filter((cls) => !!cls);

  // Exit early if no valid classes are provided after filtering
  if (classes.length === 0) {
    return [];
  }
  ...
  function dfs(el: Element) {
    // Check if the element has all the required classes
    if (el.classList && classes.every((cls) => el.classList.contains(cls))) {
      results.push(el);
    }

    // Recursively search children using a more efficient loop
    for (const child of el.children) {
      dfs(child);
    }
  }

  // Start searching from the children of the initial element
